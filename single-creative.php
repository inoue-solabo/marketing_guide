  ...
  <div class="o-flex is-single">
    <div class="o-flex__main">
      ...
      <div class="p-custom_single__bg">
        ...
        <?php
          $terms = get_the_terms(get_the_ID(), 'creative_cat');
      ...
      <?php echo do_shortcode('[myphp file="cta_creative_small_single"]'); ?>
      <div class="p-custom_single__article">
        <?php
        $prev_post = get_adjacent_post(false, '', true);
        $prev_post_terms = get_the_terms($prev_post->ID, 'creative_cat');
        if ($prev_post && $prev_post_terms && has_term($prev_post_terms[0]->term_id, 'creative_cat')):
        ?>
        ...
        <?php
          ...
          if ($terms && !is_wp_error($terms)) {
            foreach ($terms as $term) {
              if ($term->slug == 'works') {
                  ?>
                  <a href="<?=home_url(); ?>/creative/works" class="p-custom_single__article__center">一覧へ戻る</a>
                  <?php
              } elseif ($term->slug == 'magazine') {
                  ?>
                  <a href="<?=home_url(); ?>/creative/magazine" class="p-custom_single__article__center">一覧へ戻る</a>
                  <?php
              }
            }
          }
        ...
        $next_post_terms = get_the_terms($next_post->ID, 'creative_cat');
        if ($next_post && $next_post_terms && has_term($next_post_terms[0]->term_id, 'creative_cat')):
        ?>
